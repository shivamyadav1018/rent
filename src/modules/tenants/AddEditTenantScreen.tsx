...
const schema = z.object({
  ...
  moveInDate: z
    .string()
    .refine(isValidDate, 'Enter a valid move-in date (YYYY-MM-DD)'),
...
export function AddEditTenantScreen({ navigation, route }: any) {
  ...
  const [isRefreshingUnits, setIsRefreshingUnits] = useState(
    units.length === 0,
  );
  ...
  const availableUnits = useMemo(
    () =>
      units.filter(
        unit => unit.status === 'vacant' || unit.id === originalUnitId,
      ),
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      refreshAll()
        .catch(() => undefined)
        .finally(() => {
          if (isActive) setIsRefreshingUnits(false);
        });
      return () => {
        isActive = false;
      };
  ...
  useEffect(() => {
    ...
    tenantRepo
      ...
      .catch(() => {
        if (isActive)
          setLoadError(
            'Could not load tenant. Please retry or return to the list.',
          );
      })
      .finally(() => {
    ...
    try {
      ...
      try {
        if (pickedProof) {
          ...
          if (
            existingProof &&
            existingProof.storagePath !== uploaded.storagePath
          ) {
            await idProofService
              .remove(existingProof.storagePath)
              .catch(() => undefined);
          }
        } else if (removeExistingProof && existingProof) {
          await tenantRepo.updateIdProof(id, {
            mimeType: null,
            name: null,
            storagePath: null,
          });
        ...
        if (newlyUploadedPath)
          await idProofService.remove(newlyUploadedPath).catch(() => undefined);
        proofWarning =
          error instanceof Error
            ? error.message
            : 'The selected ID proof could not be uploaded.';
      }
      ...
      if (proofWarning)
        Alert.alert(
          'Tenant saved',
          `Tenant details were saved, but the ID proof was not uploaded. ${proofWarning}`,
        );
    } catch (error) {
      Alert.alert(
  ...
  const chooseIdProof = async () => {
    if (authStatus !== 'signedIn') {
      return Alert.alert(
        ...
        'Connect your Firebase account in Settings before uploading an ID proof.',
      );
    }
    try {
      ...
      Alert.alert(
        'Could not select ID proof',
        error instanceof Error
          ? error.message
          : 'Please try another image or PDF.',
      );
    }
  };
  ...
  return (
    <Screen>
      ...
      />
      <Muted>
        Assign a vacant unit and initialize your tenant’s bahi-khata ledger.
      </Muted>
      <FormSection
        title="Property & Unit"
        subtitle="Select a premise for the rent ledger"
      ...
      >
        {isRefreshingUnits && units.length === 0 ? (
              ...
              return (
                ...
                >
                  <View
                    style={[
                      styles.unitIcon,
                      selected && styles.unitIconSelected,
                    ]}
                  >
                    <AppIcon
                  ...
                  <View style={styles.unitInfo}>
                    <Muted numberOfLines={1} style={styles.propertyName}>
                      {unit.property_name}
                    </Muted>
                    <Body numberOfLines={1} style={styles.unitName}>
                      {unit.name}
                    </Body>
          ...
          <View style={styles.emptyUnits}>
            ...
            <Body style={styles.emptyUnitTitle}>
              {units.length === 0
        ...
        {errors.unitId?.message ? (
          <Muted style={styles.error}>{errors.unitId.message}</Muted>
        ) : null}
      </FormSection>
      <FormSection
        title="Tenant Identity"
        subtitle="KYC & contact essentials"
      ...
      <FormSection
        ...
        subtitle="Set the ledger billing schedule"
        icon="currency-inr"
      >
        <Controller
          control={control}
        ...
        <Controller
          ...
          render={({ field }) => (
            <AppDatePicker
              ...
              value={field.value}
              onChange={value =>
                setValue('moveInDate', value, { shouldValidate: true })
              }
            />
          )}
        />
      ...
      <FormSection
        ...
        subtitle="Government ID and agreement notes"
        icon="file-document-outline"
      >
        <Pressable
          accessibilityRole="button"
          accessibilityLabel="Choose government ID document"
          style={styles.upload}
          onPress={chooseIdProof}
        >
          ...
          <Body style={styles.label}>Upload Aadhaar / Government ID</Body>
          <Muted style={styles.uploadText}>JPG, PNG or PDF · up to 10 MB</Muted>
          <Muted style={styles.uploadText}>
            {authStatus === 'signedIn'
              ? 'Uploaded to your account when saved'
              : 'Connect an account to upload ID proof'}
          </Muted>
        </Pressable>
        {pickedProof ? (
          <View style={styles.proofRow}>
            ...
            />
            <View style={styles.unitInfo}>
              <Body numberOfLines={1}>{pickedProof.name}</Body>
              <Muted>Ready to upload when saved</Muted>
            </View>
            <AppButton
              title="Remove"
              variant="secondary"
              onPress={() => setPickedProof(null)}
            />
          </View>
          ...
          <View style={styles.proofRow}>
            ...
            />
            <View style={styles.unitInfo}>
              <Body numberOfLines={1}>{existingProof.name}</Body>
              <Muted>Stored securely in Firebase</Muted>
            </View>
            <AppButton
              title="Remove"
              variant="secondary"
              onPress={() => setRemoveExistingProof(true)}
            />
          </View>
        ) : removeExistingProof ? (
          <Muted>Existing ID proof will be removed when saved.</Muted>
        ) : null}
        <AppButton
          title={
            existingProof && !removeExistingProof
      ...
      <InfoNote title="Ledger Entry Preview">
        The rent cycle starts from the move-in date with your selected monthly
        due day.
      </InfoNote>
      <AppButton
